    ...
    T: Ord,
{
    ...
    fn insert(&mut self, value: T) {
        match self.root.as_mut() {
            ...
            Some(root) => match root.value.cmp(&value) {
                Ordering::Less => match root.right.as_mut() {
                    None => {
                ...
                Ordering::Equal => {}
                Ordering::Greater => match root.left.as_mut() {
                    None => {
        ...
        match self.root.as_ref() {
            ...
            Some(node) => match node.value.cmp(&value) {
                Ordering::Less => match node.right {
                    None => false,
                    Some(ref node_right) => node_right.search(&value),
                },
                Ordering::Equal => true,
